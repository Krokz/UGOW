...
# Add "ugow" to the LSM order so it loads at boot
# Find the CONFIG_LSM line and append ",ugow"
...
lock-free lookups on the hot path. Grant inheritance walks up the directory
tree, so a grant on `/mnt/c/data` covers `/mnt/c/data/sub/file.txt`.
